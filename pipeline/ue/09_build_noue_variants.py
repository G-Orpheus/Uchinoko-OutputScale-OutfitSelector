...
既存vp_ue_mat.py::make_material()(UEモードが使うフルバイクMaterial)は無改変のまま
温存する。本スクリプトは全く別名の新規アセット(恒久マスター)+一時的に
"M_VP_{slot}"という正規名で作り直すコンボ別アセットを追加するだけで、UEモードの
既存資産には一切触れない(UEモードの退行防止)。
...
再コンパイルを避ける):
  ...
  UnrealEditor-Cmd.exe <ue_project> -run=pythonscript
      -script=pipeline\\ue\\09_build_noue_variants.py -stdout -unattended -nopause -nosplash
