...
def detect_hair_chains(avatar_arm, pal_map, explicit_roots, spring_roots,
    ...
    if explicit_roots:
        ...
        for n in explicit_roots:
            ...
            if b is None:
                warnings.append(f"hair_bones: ボーンが見つからない: {n}")
            else:
                roots.append(b)
        src = "明示指定"
    elif spring_roots:
        ...
        for n in spring_roots:
            ...
            if is_head_descendant(b):
                ...
                skipped.append(n)
        src = "VRM SpringBone定義"
        if skipped:
            print(f"[{TAG}] spring skip(頭外・剛体化のまま): {skipped}")
    else:
...
def split_hair_mesh(meshes, hair_bones):
    ...
    for f in fragments:
        try:
            ...
            print(f"[{TAG}][WARN] 断片がViewLayer外: {f.name}")
    if ok == 0:
        return survivors, None
...
def attach_orphan_roots(avatar_arm, pal_map):
    ...
    if not candidates:
        print(f"[{TAG}][WARN] 孤立ルート接続: マップ済み候補ボーンが無い "
              f"({orphan_roots}) — 祖先不明のまま後段へ渡る")
        return
...
def remap_vertex_groups(obj, group_targets):
    ...
    if unmatched:
        print(f"[{TAG}][WARN] 祖先不明のグループ({obj.name}): {unmatched}")
...
def log_structure_summary(meshes, pal_arm, orig_parent_info, material_count,
    ...
    if not all_corners:
        print(f"[{TAG}][structure] メッシュ0件のためサマリを出せません")
        return
    ...
    print(f"[{TAG}][structure] メッシュ数={len(infos)} ボーン数={len(pal_arm.data.bones)} "
          f"マテリアル数={material_count} 全体bbox=({size.x:.1f},{size.y:.1f},{size.z:.1f})cm "
          f"骨格原点=({origin.x:.2f},{origin.y:.2f},{origin.z:.2f})")
    ...
    for m in infos:
        ...
        skin_desc = (f"skinned(グループ{skin['n_groups']}, 最大配分"
                     f"{skin['dominant_ratio']*100:.0f}%)" if skin["n_groups"] > 0
                     else "重み無し")
        parent_desc = ""
        if is_rigid:
            if m["orig_parent_type"] == "BONE" and m["orig_parent_bone"]:
                parent_desc = f" 元の親=ボーン'{m['orig_parent_bone']}'"
            elif m["orig_parent_type"]:
                parent_desc = f" 元の親種別={m['orig_parent_type']}"
        detail = (f"[{TAG}][structure]   - {m['name']}: 頂点{m['vcount']} {skin_desc} "
                  f"中心cm=({cx},{cy},{cz}) 原点相対距離={rel*100:.1f}%{parent_desc}")
        if dangling:
            ...
            warn_lines.append(
                f"[{TAG}][WARN][構造異常/親ボーン参照切れ] {m['name']}: 元ファイルで"
                f"ボーン'{m['orig_parent_bone']}'に直接固定(スキニングでなくボーン子)"
                f"されていましたが、変換後の骨格に同名ボーンが無いため参照が解決できて"
                f"いません。位置が意図通りに引き継がれていない可能性があります"
                f"(現在の中心座標は骨格原点から{d:.1f}cm、全体サイズの{rel*100:.1f}%。"
                f"下のプレビュー画像で位置を目視確認してください)")
            ...
            if target != ZERO_WEIGHT_FALLBACK_BONE:
                warn_lines.append(
                    f"[{TAG}][WARN][構造/非スキンメッシュ束縛] "
                    f"{m['name']}: remap後にこのメッシュへ対応するパルボーンの"
                    f"頂点グループが1つもありませんでした(元アバターでスキニング"
                    f"されていない静的メッシュ)。元の親ボーンを辿ってパル骨格の"
                    f"'{target}'へ解決し束縛したため、位置・追従はそのボーンに"
                    f"従います(現在の中心座標は骨格原点から{d:.1f}cm、全体"
                    f"サイズの{rel*100:.1f}%。下のプレビュー画像で位置を目視"
                    f"確認してください)")
            else:
                warn_lines.append(
                    f"[{TAG}][WARN][構造異常/スキニング消失(0 pal groups)] "
                    f"{m['name']}: remap後にこのメッシュへ対応するパルボーンの"
                    f"頂点グループが1つもありませんでした。元アバターのウェイトが"
                    f"新骨格のどのボーンへも引き継げず、全頂点が救済措置で"
                    f"'{ZERO_WEIGHT_FALLBACK_BONE}'へ一律固定されています。姿勢に"
                    f"追従せず、位置・向きが意図通りでない可能性が高いです"
                    f"(現在の中心座標は骨格原点から{d:.1f}cm、全体サイズの"
                    f"{rel*100:.1f}%。下のプレビュー画像で位置を目視確認して"
                    f"ください)")
            warn_lines.append(detail)
        elif rel < REL_ORIGIN_ALERT and is_rigid:
            warn_lines.append(
                f"[{TAG}][WARN][要確認/原点付近] {m['name']}: 他のメッシュに対して"
                f"不自然なほど骨格原点付近(全体サイズの{rel*100:.1f}%)にあり、かつ"
                f"スキニングが単一グループに偏っています。親を見失っている"
                f"可能性があるため目視確認を推奨します(確定情報ではありません)")
            warn_lines.append(detail)
        else:
    ...
    if len(ok_lines) <= SUMMARY_MESH_LIMIT:
        ...
        for line in ok_lines[:SUMMARY_MESH_LIMIT]:
            print(line)
        print(f"[{TAG}][structure]   ...ほか{len(ok_lines) - SUMMARY_MESH_LIMIT}件は"
              f"正常範囲(スキニング済み・原点から十分離れている)のため省略")
...
def main():
    ...
    if not os.path.exists(blend):
        core.die(TAG, f"step01の出力が無い: {blend}")
    ...
    if avatar_arm is None:
        core.die(TAG, "アバターArmatureが見つからない")
    meshes = [bpy.data.objects[n] for n in meta["meshes"]
              if n in bpy.data.objects]
    if not meshes:
        core.die(TAG, "アバターメッシュが見つからない")
    ...
    if sway_cloth:
        ...
        if missing:
            core.die(TAG, f"sway_cloth: バニラ揺れボーンが構築できていない: {missing}")
        for i, av in enumerate(sway_cloth):
            group_targets[av] = cloth_chain[min(i, len(cloth_chain) - 1)]
        print(f"[{TAG}] sway_cloth(実験): {sway_cloth} -> {cloth_chain}")
    ...
    print(f"[{TAG}] chibi_bone_world_head: {len(_chibi_bone_world_head)}ボーン "
          f"-> {_chibi_dump_path}")
    ...
    if job.get("hair_sway", True):
        if not os.path.exists(hair_refskel):
            print(f"[{TAG}][WARN] refskel_hair.jsonが無い — 揺れ髪は無効"
                  "(バニラ抽出のやり直しが必要)")
        else:
            local_warn = []
    ...
    for obj in meshes:
        ...
        if src_bone:
            ...
            if resolved:
                ...
                print(f"[{TAG}][WARN] 非スキンメッシュ {obj.name}: 元の親ボーン"
                      f"'{src_bone}'がパル骨格へ対応せず、"
                      f"'{ZERO_WEIGHT_FALLBACK_BONE}'へ救済")
