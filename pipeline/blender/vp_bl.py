...
def load_job_from_argv():
    args = blender_argv()
    if not args:
        core.die("vp_bl", "ジョブJSONが指定されていない(-- job.json)")
...
def humanoid_map_from_json(arm, json_path, warnings):
    ...
    if hips_name and resolve(hips_name) is None:
        ...
        if obj_base in (hips_name, _compat_name(hips_name)):
            ...
            if hl:
                ...
                head = (0.0, 0.0, 0.0)
                warnings.append("hips_localが無いため原点に復元(古いhumanoid.json。"
                                "位置がずれる場合はUnity輸出をやり直すこと)")
            bpy.context.view_layer.objects.active = arm
            bpy.ops.object.mode_set(mode="EDIT")
            ...
            for b in list(arm.data.edit_bones):
                if b.parent is None and b.name != hips_name:
                    b.parent = eb
            bpy.ops.object.mode_set(mode="OBJECT")
            warnings.append(f"スケルトンルート復元: {hips_name} head={head}"
                            "(FBXのeRootがアーマチュア化されていた)")
    ...
    for human_name, bone_name in human.items():
        ...
        if actual is not None:
            ...
            warnings.append(f"humanoid.json: ボーンがFBXに無い: "
                            f"{human_name} -> {bone_name}")
    if "pelvis" not in pal_map or "head" not in pal_map:
        die("vp_bl", "humanoid.jsonの必須ボーン(Hips/Head)が解決できない")
    return pal_map
...
def humanoid_to_pal_map(arm):
    ...
    die("vp_bl", "VRM Humanoid定義が読めない(hips/headが未割当)。"
        "VRMファイルが壊れているかアドオン非対応の形式")
