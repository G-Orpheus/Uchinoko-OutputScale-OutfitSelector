...
def log_image_alpha(image, material_name, output_name):
    """Blenderへ読み込まれた元画像のAlpha統計をstep01ログへ出す。"""
    try:
        import numpy as np
        channels = int(image.channels)
        count = int(image.size[0]) * int(image.size[1])
        if channels < 4 or count <= 0:
            print(f"[{TAG}] Alpha元テクスチャ '{material_name}' -> {output_name}: "
                  "Alphaチャンネル無し(255扱い)")
            return
        pixels = np.empty(count * channels, dtype=np.float32)
        image.pixels.foreach_get(pixels)
        a = pixels[3::channels]
        print(f"[{TAG}] Alpha元テクスチャ '{material_name}' -> {output_name}: "
              f"min={float(a.min()):.4f} max={float(a.max()):.4f} "
              f"透明={int((a <= (0.5 / 255.0)).sum())} "
              f"半透明={int(((a > (0.5 / 255.0)) & (a < (254.5 / 255.0))).sum())} "
              f"pixels={a.size}")
    except Exception as e:
        print(f"[{TAG}][WARN] Alpha統計を取得できない: {material_name} ({e})")
...
def _matte_rgb():
    ...
    parts = raw.split(",")
    if len(parts) != 3:
        print(f"[{TAG}][WARN] D2P_MATTE_COLOR不正(R,G,B形式で指定): {raw!r} — 無視")
        return None
    try:
        r, g, b = (max(0, min(255, int(p.strip()))) for p in parts)
    except ValueError:
        print(f"[{TAG}][WARN] D2P_MATTE_COLOR不正(整数でない): {raw!r} — 無視")
        return None
    return (r, g, b)
...
def apply_matte_if_enabled(path):
    ...
    if not path.lower().endswith(".png"):
        print(f"[{TAG}][WARN] マット化はPNGのみ対応のためスキップ: {path}")
        return
    size = _png_size(path)
    if size is None:
        print(f"[{TAG}][WARN] PNGヘッダ解析に失敗、マット化スキップ: {path}")
        return
    w, h = size
    r, g, b = color
    sig = b"\x89PNG\r\n\x1a\n"
    ...
    with open(path, "wb") as f:
        f.write(png)
    print(f"[{TAG}] D2P_MATTE_COLOR適用: {path} を単色RGBA({r},{g},{b},255)で"
          f"上書き ({w}x{h})")
...
def import_vrm(job):
    ...
    if arm is None:
        core.die(TAG, "VRMインポートでArmatureが得られない")
    return arm
...
def import_fbx_avatar(job, warnings, humanoid_json=None):
    ...
    if flattened:
        ...
        warnings.append("D2P平坦化FBX: メッシュ行列をFBXルートへ固定")
        if unskinned:
            warnings.append(
                "D2P平坦化FBX: 非スキンメッシュ(ボーン/末端ノード直付け)は"
                f"位置を維持: {[o.name for o in unskinned]}")
    height = 0.0
    for o in meshes:
    ...
    if height > 10.0:  # 高さ10m超の人型はいない=cm単位と判断(Unity FBX Exporterはcm)
        ...
        warnings.append(f"単位をcm→mへ正規化(正規化前の高さ {height:.0f})")
    ...
    if single_usered:
        warnings.append(f"multi-userメッシュを単一ユーザー化: {single_usered}")
...
def find_humanoid_json(job):
    ...
    for cand in (stem + ".humanoid.json",
        ...
        if os.path.exists(cand):
            return cand
    core.die(TAG, "humanoid.json が見つかりません。FBX入力にはボーン対応表が必要です。\n"
             "Unityでアバターを開き、同梱の unity\\HumanoidMapExporter.cs "
             "(Assets/Editorに配置)のメニュー Tools > DiveToPalworld > "
             "Export Humanoid Map で書き出し、FBXと同じフォルダに置いてください")
    ...
    for obj in list(bpy.data.objects):
        ...
        if obj.type == "MESH" and (obj.parent is arm or any(
            ...
            if obj.hide_viewport or obj.hide_render:
                warnings.append(f"非表示メッシュを除外: {obj.name}")
...
def drop_bone_meshes(arm, meshes, drop_bones, warnings):
    ...
    for name in drop_bones:
        ...
        if bone is None:
            warnings.append(f"drop_bones: ボーンが見つからない: {name}")
            continue
    ...
    if not targets:
        return meshes
    print(f"[{TAG}] drop_bones: {sorted(drop_bones)} -> 対象{len(targets)}本(子孫込み)")
    ...
    for obj in meshes:
        ...
        if len(sel) == len(obj.data.vertices):
            name = obj.name
            bpy.data.objects.remove(obj, do_unlink=True)
            print(f"[{TAG}] drop_bones: メッシュごと削除: {name}")
            continue
        bpy.ops.object.select_all(action="DESELECT")
        obj.select_set(True)
        ...
        bpy.ops.object.mode_set(mode="OBJECT")
        print(f"[{TAG}] drop_bones: {obj.name}: {len(sel)}頂点削除")
        if len(obj.data.vertices) == 0:
            bpy.data.objects.remove(obj, do_unlink=True)
        else:
            survivors.append(obj)
    if not survivors:
        core.die(TAG, "drop_bonesで全メッシュが消えた(指定が広すぎる)")
    return survivors
...
def extract_materials_from_unity_map(meshes, orig_names, tex_dir, fbx_dir,
    ...
    for obj in meshes:
        ...
        if entries is None:
            warnings.append(f"material_map.jsonに無いメッシュ: {orig}(単色になります)")
            entries = []
        ...
        if len(obj.material_slots) < len(entries):
            warnings.append(f"スロット数が合わない: {orig} "
...
def extract_materials(meshes, tex_dir, warnings, search_dirs):
    ...
    for obj in meshes:
        if not obj.data.materials:
            ...
            warnings.append(f"マテリアル無しメッシュに灰色を割当: {obj.name}")
        ...
        if image is not None:
            if image.name in saved_images:
                ...
                if src is not None:
                    ...
                    warnings.append(f"テクスチャが見つからない: {orig} "
                                    f"({image.name}) — 単色になります。"
                                    "画像ファイルをFBXと同じフォルダに置いて再実行してください")
            if tex_file is not None:
                log_image_alpha(image, orig, tex_file)
        mat.name = slot
        slots[slot] = {
...
def main():
    ...
    if not os.path.exists(job["vrm_path"]):
        core.die(TAG, f"VRMが無い: {job['vrm_path']}")
    ...
    # 入口での正規化(公開issue #18): 制作者が編集時に表示を切ったまま保存した
    # Armatureモディファイア(show_viewport/show_render=False)を強制ONにする。
    # 放置するとstep02のmodifier_applyがBlender自身のRuntimeError
    # (「モディファイアーはOFFです」)で停止する。エラーにせずログを残して進む。
    for mesh_name, _mod, flags in vp_modnorm.normalize_armature_modifiers(
            meshes, tag=TAG):
        warnings.append(
    ...
    for req in ("pelvis", "spine_01", "head", "upperarm_l", "upperarm_r",
        ...
        if req not in pal_map:
            core.die(TAG, f"必須Humanoidボーンが未割当: {req}")
    if "clavicle_l" not in pal_map:
        warnings.append("肩(shoulder)ボーンが無い — 肩スライダーは上腕に適用")
