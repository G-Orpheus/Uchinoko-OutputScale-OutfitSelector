...
def _read_unversioned_header(data, off):
    ...
    while True:
        if off + 2 > len(data):
            raise MatParamError("unversioned header が途中で終わっている")
        ...
        if frags[-1][3]:
            break
        if len(frags) > 256:
            raise MatParamError("unversioned header のフラグメントが多すぎる")
...
def _write_unversioned_header(slots):
    ...
    if [s[0] for s in slots] != list(range(len(slots))):
        raise MatParamError("連続 index 以外のヘッダ生成は未対応")
    n = len(slots)
    if not 1 <= n <= 127:
        raise MatParamError(f"値の個数 {n} は未対応")
...
def _parse_param_entry(data, off, value_size):
    ...
    if [s[0] for s in slots] != [0, 1, 2]:
        raise MatParamError(
            f"@{off}: パラメータ要素の値配置 {slots} は未対応"
            f"(ParameterInfo/ParameterValue/ExpressionGUID の3値のみ対応)")
    if slots[0][1]:
        raise MatParamError(f"@{off}: ParameterInfo がゼロ省略されている(未対応)")

    pslots, off3, pihdr = _read_unversioned_header(data, off2)
    if [s[0] for s in pslots] != [0, 1, 2] or any(z for (_i, z) in pslots):
        raise MatParamError(
            f"@{off}: ParameterInfo の値配置 {pslots} は未対応"
            f"(Name/Association/Index の3値が全て提示されている形のみ対応)")
    ...
    if slots[1][1]:
        ...
        if len(value) != value_size:
            raise MatParamError(f"@{off}: ParameterValue が足りない")
        p += value_size
    ...
    if slots[2][1]:
        ...
        if len(guid) != _GUID_SIZE:
            raise MatParamError(f"@{off}: ExpressionGUID が足りない")
        p += _GUID_SIZE
...
def _build_param_entry(entry, value_size):
    ...
    if value is not None and len(value) != value_size:
        raise MatParamError("ParameterValue のサイズ不一致")
    if guid is not None and len(guid) != _GUID_SIZE:
        raise MatParamError("ExpressionGUID のサイズ不一致")
...
def _parse_param_array(data, off, value_size):
    ...
    if not (0 <= count <= 512):
        raise MatParamError(f"@{off}: 要素数 {count} が異常")
    p = off + 4
    entries = []
...
def parse_mi_uexp(uexp_bytes):
    ...
    for (idx, is_zero) in slots:
        ...
        if idx > PROP_TEXTURE:
            break  # 以降は未解釈のまま残す
        raise MatParamError(
            f"未知のトップレベルプロパティ index={idx} @{off} "
            f"(このMIは本モジュールの想定外の形)")
    if PROP_TEXTURE not in arrays:
        raise MatParamError("TextureParameterValues が見つからない")
    return dict(arrays=arrays, order=order, tail_start=off)


def list_texture_parameters(uasset_bytes, uexp_bytes):
    """MIのTextureParameterValuesを名前と完全パッケージパスで返す。

    変換ログと検査で共用し、Base Color以外に元防具テクスチャ参照が
    残っていないことをバイナリから確認できるようにする。
    """
    h = _lt._parse_header_with_offsets(uasset_bytes)
    names, names_end = _lt._read_name_table(
    ...
    for _ in range(h.import_count):
        imp, off = _lt._parse_import(uasset_bytes, off)
        imports.append(imp)
    mi = parse_mi_uexp(uexp_bytes)
    result = []
    for entry in mi["arrays"][PROP_TEXTURE]["entries"]:
        if not (0 <= entry["name_idx"] < len(names)):
            pname = f"<name:{entry['name_idx']}>"
        else:
            pname = names[entry["name_idx"]]
        fpi = struct.unpack("<i", entry["value"])[0] if entry["value"] else 0
        package_path = None
        if fpi < 0:
            tex_i = -fpi - 1
            if 0 <= tex_i < len(imports):
                tex_imp = imports[tex_i]
                outer = tex_imp["outer_index"]
                pkg_i = -outer - 1 if outer < 0 else -1
                if 0 <= pkg_i < len(imports):
                    pkg_imp = imports[pkg_i]
                    if 0 <= pkg_imp["object_name_idx"] < len(names):
                        package_path = names[pkg_imp["object_name_idx"]]
        result.append({"name": pname, "path": package_path, "fpi": fpi})
    return result


def list_scalar_parameters(uasset_bytes, uexp_bytes):
    """MIのScalarParameterValuesを{name, value}で返す。"""
    h = _lt._parse_header_with_offsets(uasset_bytes)
    names, names_end = _lt._read_name_table(
        uasset_bytes, h.name_offset, h.name_count)
    if names_end != h.import_offset:
    ...
    for entry in arr["entries"]:
        if not (0 <= entry["name_idx"] < len(names)):
            ...
            pname = names[entry["name_idx"]]
        value = struct.unpack("<f", entry["value"])[0] \
            if entry["value"] is not None else 0.0
        result.append({"name": pname, "value": value})
    return result


def material_parent_path(uasset_bytes, uexp_bytes):
    """MaterialInstanceConstantのParent完全パッケージパスを返す。"""
    h = _lt._parse_header_with_offsets(uasset_bytes)
    names, names_end = _lt._read_name_table(
        uasset_bytes, h.name_offset, h.name_count)
    if names_end != h.import_offset:
        raise MatParamError("name table/import offset不一致")
    imports = []
    off = h.import_offset
    for _ in range(h.import_count):
        imp, off = _lt._parse_import(uasset_bytes, off)
        imports.append(imp)
    slots, payload, _raw = _read_unversioned_header(uexp_bytes, 0)
    parent_fpi = None
    for idx, is_zero in slots:
        if is_zero:
            continue
        if idx == 10:
            parent_fpi = struct.unpack_from("<i", uexp_bytes, payload)[0]
            break
        if idx in _SIMPLE_PROP_SIZE:
            payload += _SIMPLE_PROP_SIZE[idx]
        else:
            break
    if parent_fpi is None or parent_fpi >= 0:
        return None
    imp_i = -parent_fpi - 1
    if not (0 <= imp_i < len(imports)):
        return None
    outer = imports[imp_i]["outer_index"]
    pkg_i = -outer - 1 if outer < 0 else -1
    if not (0 <= pkg_i < len(imports)):
        return None
    name_i = imports[pkg_i]["object_name_idx"]
    return names[name_i] if 0 <= name_i < len(names) else None
...
def edit_material_instance(uasset_bytes, uexp_bytes, ops):
    ...
    if h.export_count != 1:
        raise MatParamError(
            f"export_count={h.export_count}(単一exportのMIのみ対応)")
    ...
    for op in ops:
        ...
        if kind == "scalar":
            ...
            if arr is None or not arr["entries"]:
                raise MatParamError("ScalarParameterValues が無い/空")
            ...
            if arr is None or not arr["entries"]:
                raise MatParamError("VectorParameterValues が無い/空")
            e = _find_entry(arr, nt.names, pname)
            if e is None:
                e = _clone_entry(arr["entries"][0])
                e["name_idx"] = nt.get(pname)
            ...
            if arr is None:
                raise MatParamError("TextureParameterValues が無い")
            if _find_entry(arr, nt.names, "Emissive Texture") is not None:
                log.append("[emissive] 既に存在")
                continue
            bt = _find_entry(arr, nt.names, "Base Texture")
            if bt is None:
                raise MatParamError('"Base Texture" が無い')
            e = _clone_entry(bt)
            ...
            raise MatParamError(f"未知のop: {kind}")
