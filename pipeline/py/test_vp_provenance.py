...
def test_write_build_provenance():
    ...
    check("write_build_provenance: 戻り値パスにファイルが実在",
    ...
    check("avatar_nameが引数そのまま", data.get("avatar_name") == "Dummy")
    check("engine_modeがnoue固定", data.get("engine_mode") == "noue")
    check("pak_filenameがbasename一致",
          data.get("pak_filename") == os.path.basename(out_pak))
    check("template_sourceが引数そのまま", data.get("template_source") == "dev_fallback")
    check("git_dirtyがbool型", isinstance(data.get("git_dirty"), bool))
