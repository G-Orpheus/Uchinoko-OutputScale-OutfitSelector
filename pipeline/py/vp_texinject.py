...
(pipeline\\ue\\vp_ue_mat.py make_material())が持つ
...
# 導出(docs\REPORT_U47_2026-07-25.md 1.6節+pipeline\ue\vp_ue_mat.py):
...
def shadow_lift_gain(shadow_lift, unlit=False, l0=None):
    ...
    unlit=TrueならUE版M_VPもEmissive経路のみ(k=0扱い、pipeline\\ue\\vp_ue_mat.py
...
def inject_texture_file(template_uexp_path, png_path, out_uexp_path, alpha_coverage=False,
    ...
    # Base ColorのAlphaをそのまま維持する。t00/t01はPF_DXT1だが、
    # vp_tex.encode_dxt1が3色+1bit透明モードを実装しているため、Masked用途の
    # Alpha=0領域は保持できる。半透明値はMaskedの閾値(128)で二値化される。
    input_alpha = vp_tex.alpha_stats(rgba)
    ...
    for m, img in zip(layout["mips"], mips):
        ...
        if len(blob) != m["size"]:
            raise RuntimeError(
                f"エンコードサイズ不一致 {m['w']}x{m['h']}: {len(blob)} != {m['size']}")
    ...
    output_alpha = vp_tex.alpha_stats(decoded0)

    os.makedirs(os.path.dirname(out_uexp_path) or ".", exist_ok=True)
    with open(out_uexp_path, "wb") as f:
        f.write(data)
    print(f"[vp_texinject] Alpha 入力 min={input_alpha['min']} max={input_alpha['max']} "
          f"透明={input_alpha['transparent']} 半透明={input_alpha['partial']} / "
          f"最終{pf} min={output_alpha['min']} max={output_alpha['max']} "
          f"透明={output_alpha['transparent']}")
    return {"pixel_format": pf, "size_x": sx, "size_y": sy, "psnr": psnr,
            "gain": gain, "gain_version": TEXINJECT_GAIN_VERSION,
            "input_alpha": input_alpha, "output_alpha": output_alpha,
            "blend_mode": "Masked", "opacity_mask": "Base Texture.A",
