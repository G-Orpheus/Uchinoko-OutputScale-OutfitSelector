...
def pack_section(global_strip, class_strip, material_index, base_index, num_triangles,
    ...
    if len(dup_vert_index_data) != num_vertices:
        raise AvatarBuildError(
            f"dup_vert_index_dataの長さ({len(dup_vert_index_data)})がnum_vertices"
            f"({num_vertices})と不一致")
    out = bytearray()
    ...
    for path in candidates:
        if os.path.exists(path):
            with open(path, encoding='utf-8') as f:
                raw = json.load(f)
            output_scale = float(dump.get('output_scale', 1.0))
            return {k: tuple(coord * output_scale for coord in v)
...
def patch_refskeleton_positions(data, uexp_path, uasset_path, bones, avatar_world_pos):
    ...
    for name, pos in patched.items():
        ...
        n_patched += 1
    print(f"[build_avatar_variant] U21 RefSkeletonバインドポーズ位置パッチ: "
          f"{n_patched}/{len(patched)}ボーン(chibi-fit後の値に一致させた)")
    return bytes(out)
...
def build_uexp_variant(uexp_path, uasset_path, dump):
    ...
    if dump.get('format') != 2:
        raise AvatarBuildError(
            f"dump format=2(material付き)が必要(実際={dump.get('format')})。"
            "T1のdump_avatar_mesh.pyで再生成せよ")
    ...
    for t in tris:
        if len(t) != 4:
            raise AvatarBuildError(f"三角形がmaterial付き4要素でない: {t}")
        m = t[3]
        if m not in (0, 1):
            raise AvatarBuildError(f"未対応material値: {m}")
        tris_by_mat[m].append(t[:3])
    if tris_by_mat[1]:
        print(f"[build_avatar_variant] 注記: material=1の三角形{len(tris_by_mat[1])}件を"
              "material=0へ畳む(U50-single: 単一マテリアル構成)")
        tris_by_mat[0] += tris_by_mat[1]
        tris_by_mat[1] = []
    if not tris_by_mat[0]:
        raise AvatarBuildError("三角形が0件")
    ...
    if dump_max_inf is not None and dump_max_inf != max_bone_influences:
        print(f"[build_avatar_variant] 注記: ダンプのmax_influences({dump_max_inf})と"
              f"元ファイルのMaxBoneInfluences({max_bone_influences})が不一致 "
              "(エンコード時にファイル側の値へ切り詰めるため問題なし)")
    ...
    if 0 not in sections_by_material:
        raise AvatarBuildError(
            f"テンプレートSKにMaterialIndex=[0]のセクションが無い"
            f"(実際のMaterialIndex集合={sorted(sections_by_material)})。"
            "本ビルダーはMaterialIndex 0(body)のセクションが必須")
    ...
    for i, (m, avatar_mats, old_sec) in enumerate(output_plan):
        ...
        if unknown:
            raise AvatarBuildError(f"RefSkeletonに存在しないボーン名: {sorted(unknown)}")
        bone_map = sorted(bone_names.index(n) for n in used_names)
        if len(bone_map) > 65535:
            raise AvatarBuildError(f"セクション{i}の使用ボーン数が上限を超える: {len(bone_map)}")
    ...
    if bvar or buse16:
        raise AvatarBuildError("元ファイルがbVariableBonesPerVertex/bUse16BitBoneIndexを使用している(未対応形式)")
...
def build_uasset_variant(uasset_path, new_uexp_size):
    ...
    if len(asset_exports) != 1:
        raise AvatarBuildError(
            f"bIsAsset=Trueのexportが一意でない: {len(asset_exports)}件"
            f"(全ExportCount={len(exports)}件)")
    export = asset_exports[0]
    prefix = export['serial_offset'] - len(data)
    if prefix < 0:
        raise AvatarBuildError(
            f"本体exportのSerialOffset({export['serial_offset']})がuassetサイズ"
            f"({len(data)})未満")
