...
RETARGET_JOB_KEYS = (
    "avatar_name", "shoulder_offset_deg", "output_scale", "merge_fingers", "merge_eyes",
...
def find_bundled_python(blender_exe):
    ...
    if not cands:
        die(f"Blender同梱Pythonが見つからない(Blenderフォルダ配下に "
            f"*/python/bin/python.exe が無い): {os.path.dirname(blender_exe)}")
    return cands[0]
...
def read_vanilla_version():
    ...
    die(f"extract_vanilla.pyからVANILLA_VERSIONを読めない: {path}")
...
def main():
    ...
    if recipe.get("format") != "d2p-sanitized-recipe-1":
        die(f"未知のrecipe形式: {recipe.get('format')}")
    retarget_job_params = recipe.get("retarget_job_params") or {}
    if not retarget_job_params:
        die("recipeにretarget_job_paramsが無い。sanitize_pak.py --strip-vertices を "
            "--retarget-job 付きで実行して作ったrecipe.jsonが必要")
    correspondence = recipe.get("vertex_correspondence") or {}
    genders = sorted(correspondence.keys())
    if not genders:
        die("recipeにvertex_correspondenceが無い(頂点0フィルされていないrecipe)")
    print(f"[{TAG}] 復元対象の性別: {genders}")
    print(f"[{TAG}] 再ターゲットパラメータ(recipe由来): {retarget_job_params}")
    ...
    print(f"[{TAG}] job.json生成: {job_path}")
    ...
    if have_version == vanilla_version:
        print(f"[{TAG}] === Phase 0: バニラ情報抽出 — キャッシュ一致(version {have_version})、スキップ ===")
    elif args.skip_vanilla and have_version:
        print(f"[{TAG}] === Phase 0: --skip-vanilla指定、既存version {have_version} を再利用 ===")
    else:
        print(f"[{TAG}] === Phase 0: バニラ情報抽出 ===")
    ...
    print(f"[{TAG}] === Phase 1: step01 (アバター取込) ===")
    ...
    for gender in genders:
        ...
        if not os.path.exists(avatar_fbx):
            die(f"step03の出力が無い(step01〜03のどこかで失敗): {avatar_fbx}")

        # === Phase 2: ジオメトリダンプ(devtools/dump_restore_geometry.py) ===
        print(f"[{TAG}] === Phase 2: 復元ジオメトリダンプ ({gender}) ===")
    ...
    if not os.path.exists(out_pak):
        die(f"復元pakが生成されなかった: {out_pak}")
    print(f"[{TAG}] done: {out_pak}")
    print(f"[{TAG}] 作業ディレクトリ(job.json/ログ/中間生成物): {work}")
