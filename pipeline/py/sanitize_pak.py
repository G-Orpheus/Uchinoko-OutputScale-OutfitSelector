...
def build_pak(unrealpak, tree, pak_path, rsp_path):
    ...
    if r.returncode != 0:
        vp_core.die(TAG, f"UnrealPak失敗 exit={r.returncode}: {pak_path}\n"
                    + (r.stdout or "")[-2000:])
    return len(lines)
...
def main():
    ...
    if args.strip_vertices and not (args.avatar_tangent_json_male
        ...
        vp_core.die(TAG, "--strip-vertices には --avatar-tangent-json-male / "
                    "-female の少なくとも一方が必須(Plan B頂点対応表の構築に使う。"
                    "Male/Female両方の衣装SKがpakにあるなら両方必要)")

    os.makedirs(args.out, exist_ok=True)
    textures = find_textures(args.extract)
    if not textures:
        vp_core.die(TAG, f"テクスチャuexpが見つからない: {args.extract}")
    print(f"[{TAG}] テクスチャ {len(textures)}件: "
    ...
    for rel, info in textures:
        ...
        if os.path.exists(png):
            ...
            if (w, h) != (info["size_x"], info["size_y"]):
                print(f"[{TAG}][warn] {base}.png解像度 {w}x{h} != "
                      f"cooked {info['size_x']}x{info['size_y']}")
            pixel = vp_tex.pixel_sha1(rgba)
        else:
            print(f"[{TAG}][warn] 元PNGが無い({png})— 無改変検知が効かない")
    ...
    if args.strip_vertices:
        ...
        if not meshes:
            vp_core.die(TAG, f"Outfit SK uexpが見つからない: {args.extract}")
        print(f"[{TAG}] Outfit SK {len(meshes)}件: "
        ...
        if args.retarget_job:
            ...
            retarget_job_params = {
                k: _job[k] for k in (
                    "avatar_name", "shoulder_offset_deg", "output_scale", "merge_fingers",
        ...
        for rel, info in meshes:
            ...
            if gender not in avatar_items_by_gender:
                vp_core.die(TAG, f"{rel}: gender={gender}のavatar-tangent-jsonが"
                            "指定されていない")
            ...
            if gender not in vertex_correspondence:
                ...
                if n_missing:
                    print(f"[{TAG}][warn] 頂点対応表({gender}): {n_missing}/{len(table)} 件が"
                          f"avatar側で1cm以内に見つからない({rel}基準)")
                ...
                print(f"[{TAG}] 頂点対応表構築({gender}): "
                      f"{len(table) - n_missing}/{len(table)} 件(基準: {rel})")
            ...
            if used:
                ...
                try:
                    ...
                    print(f"[{TAG}][warn] BoneMap特定失敗({rel}): {e}")
            ...
            print(f"[{TAG}] 0フィル(頂点): {rel} (verts={info['num_vertices']})")
