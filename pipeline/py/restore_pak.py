...
def find_entry(entries, rel):
    ...
    if len(cands) != 1:
        vp_core.die(TAG, f"pak内エントリを特定できない: {rel} (候補{len(cands)})")
    return cands[0]
...
def restore_geometry_bytes(uexp_bytes, bone_names, bone_map, avatar_objects,
    ...
    if len(table) != numv:
        vp_core.die(TAG, f"対応表の頂点数不一致: table={len(table)} cooked={numv}")
    if not bone_map:
        vp_core.die(TAG, "recipeにbone_mapが無い(sanitize_pak.pyを最新版で再実行してください)")
...
def main():
    ...
    if not args.inject_original and not args.png_dir:
        vp_core.die(TAG, "--png-dir か --inject-original のどちらかが必要")

    with open(args.recipe, encoding="utf-8") as f:
        recipe = json.load(f)
    if recipe.get("format") != "d2p-sanitized-recipe-1":
        vp_core.die(TAG, f"未知のrecipe形式: {recipe.get('format')}")
    ...
    if (meshes and not args.inject_original
        ...
        vp_core.die(TAG, "meshesを含むrecipeは --inject-original(開発検証用)か "
                    "--restore-geometry-male/-female(Plan B本番復元)のどちらかが必要")
    ...
    for tex in recipe["textures"]:
        ...
        e = entries[key]
        if e["compression"] != 0:
            vp_core.die(TAG, f"エントリが非圧縮でない: {key}")
        if e["size"] != tex["uexp_size"]:
            vp_core.die(TAG, f"uexpサイズ不一致: {key} "
        ...
        if args.inject_original:
            ...
            if len(data) != tex["uexp_size"]:
                vp_core.die(TAG, f"元uexpサイズ不一致: {src}")
            for m in tex["mips"]:
            ...
            if not os.path.exists(png):
                vp_core.die(TAG, f"テクスチャが足りない: {tex['source_png']} "
                            f"({args.png_dir} に置いてください)")
            w, h, rgba = vp_tex.decode_png(png)
            if (w, h) != (tex["size_x"], tex["size_y"]):
                vp_core.die(TAG, f"{tex['source_png']}: 解像度 {w}x{h} が"
                            f"想定 {tex['size_x']}x{tex['size_y']} と違う")
            ...
            for m, img in zip(tex["mips"], mips):
                ...
                if len(blob) != m["size"]:
                    vp_core.die(TAG, f"エンコードサイズ不一致 {m['w']}x{m['h']}: "
                                f"{len(blob)} != {m['size']}")
    ...
    if (args.restore_geometry_male or args.restore_geometry_female) and not correspondence_by_gender:
        vp_core.die(TAG, "recipeにvertex_correspondenceが無い"
                    "(sanitize_pak.py --strip-vertices を最新版で再実行してください)")
    ...
    for mesh in meshes:
        ...
        if args.inject_original:
            ...
            hoff = e["offset"] + 28
            pak[hoff:hoff + 20] = hashlib.sha1(restored).digest()
            print(f"[{TAG}] 注入(頂点): {mesh['entry']} (verts={mesh['num_vertices']})")
        else:
            ...
            if restore_geo is None or correspondence is None:
                vp_core.die(TAG, f"{mesh['entry']}: gender={gender}の"
                            "--restore-geometry-{male,female}またはrecipeの"
                            "vertex_correspondenceが無い")
            ...
            print(f"[{TAG}] 注入(頂点/PlanB,{gender}): {mesh['entry']} "
                  f"({n_matched}/{numv} 頂点)")

    with open(args.out, "wb") as f:
        f.write(pak)
    mode = "元バイト注入" if args.inject_original else "PNGエンコード注入"
    print(f"[{TAG}] done({mode}): {args.out}")
    if args.inject_original:
        print(f"[{TAG}] バイト同一検証(テクスチャ): {verified}/{n} PASS")
        if meshes:
            print(f"[{TAG}] バイト同一検証(メッシュ): {mesh_verified}/{len(meshes)} PASS")
    else:
        if unmodified:
            print(f"[{TAG}] 無改変スロット: {unmodified}(改変済みが1枚以上あるため許可)")
        if args.restore_geometry_male or args.restore_geometry_female:
            print(f"[{TAG}] Plan B頂点復元: {mesh_verified}/{len(meshes)} メッシュ処理"
                  "(バイト同一検証は不可。位置誤差等はdevtools/test_planb_restore.py参照)")
