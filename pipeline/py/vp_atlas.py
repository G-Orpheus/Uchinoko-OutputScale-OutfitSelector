...
# ここを超えるはみ出しは切り詰めず、`vp_atlas_uvbake.py` の Pass 3
# (セル包含チェック)にそのまま突き当てて**ビルドを止める**
# ——見た目が壊れたものを黙って出さないための最後の砦であり、緩めてはならない。
...
def plan_avatar(meta):
    ...
    for slot_id in sorted(slots.keys()):
        ...
        if (info.get("alpha_mode") or "").upper() in ("MASK", "BLEND"):
