...
def gender_of(fn):
    ...
    if '_Female_' in fn:
        return 'Female'
    raise AllBuildError(f"ファイル名から性別を判定できない(_Male_/_Female_不在): {fn}")
...
def build_and_validate(uexp_path, uasset_path, dump, out_uexp, out_uasset):
    ...
    if s2['num_sections'] != len(expected_mat_indices):
        ...
        if mat_indices != expected_mat_indices:
            ok = False
            errs.append(f"section material_index順序={mat_indices} expected={expected_mat_indices}")
...
def build_pak(unrealpak, extract_root, all_dir, pak_path, rsp_path, targets):
    ...
    for uexp_rel in targets:
        ...
        if not (os.path.exists(u1) and os.path.exists(u2)):
            raise AllBuildError(f"改変体が見つからない: {u1} / {u2}")
    ...
    if n_replaced != len(replace_map):
        raise AllBuildError(f"差し替え対象が{len(replace_map)}件見つかるはずが実際{n_replaced}件")
    ...
    if r.returncode != 0:
        raise AllBuildError(f"UnrealPak失敗 exit={r.returncode}\n{(r.stdout or '')[-3000:]}\n{(r.stderr or '')[-1000:]}")
...
def main():
    ...
    print(f"\n改変体生成+検証: {len(pairs)}体中 "
          f"{'全数PASS' if all_ok else f'{n_fail}件FAIL'} "
    ...
    print(f"pak生成: {args.pak_out} (総エントリ{n_lines}件, 差し替え{n_replaced}件)")
