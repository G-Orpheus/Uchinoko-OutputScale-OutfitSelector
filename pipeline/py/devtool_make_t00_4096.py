...
配布物にもエンドユーザーの手順にも一切現れません。`pipeline\\ue\\
09_build_noue_variants.py`(noue用マテリアル資産をUEで焼くツール)と同じ
...
def _find_pf(data):
    ...
    raise UpscaleError("PF_文字列が見つからない(Texture2Dのuexpではない)")
...
def upscale_texture(uasset, uexp, target=TARGET_SIZE):
    ...
    if lay["size_x"] != lay["size_y"]:
        raise UpscaleError(f"正方形テクスチャのみ対応: {lay['size_x']}x{lay['size_y']}")
    if target <= old:
        raise UpscaleError(f"拡大のみ対応: {old} -> {target}")
    if target % old != 0:
        raise UpscaleError(f"整数倍のみ対応: {old} -> {target}")
    if pf not in vp_core.TEX_FORMATS:
        raise UpscaleError(f"未対応のピクセルフォーマット: {pf}")
    ...
    for mi in range(num_mips):
        ...
        p += 12
        if z != 1:
            raise UpscaleError(f"mip{mi}: z={z}(未対応)")
    ...
    for (size64, count, size_on_disk, payload, w, h, z) in entries:
        if len(payload) != count:
            raise UpscaleError(f"payload長不一致 {w}x{h}: {len(payload)} != {count}")
    ...
    if chk["size_x"] != target or chk["size_y"] != target:
        raise UpscaleError("SizeX/SizeYの書き換えに失敗した")
    if len(chk["mips"]) != new_num_mips:
        raise UpscaleError("ミップ数が合わない")
    if chk["uasset_size"] != len(new_uasset):
        raise UpscaleError(
            f"abs_offの基準がuassetサイズと合わない: {chk['uasset_size']} != {len(new_uasset)}")
    if chk["mips"][0]["w"] != target:
        raise UpscaleError("mip0の寸法が合わない")
...
def main():
    ...
    print(f"[t00-4096] {info['old']} -> {info['new']} / mips {info['mips'][0]} -> "
          ...
          f"ImportedSize更新={info['imported_size_patched']}")
