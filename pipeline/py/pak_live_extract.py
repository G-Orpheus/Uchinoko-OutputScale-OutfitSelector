...
def _resolve_ooz_python():
    ...
    if py_launcher and _is_app_execution_alias(py_launcher):
        print(f"[{TAG}] py ランチャーはWindowsのアプリ実行エイリアス"
              f"(実体なし)なので候補から除外: {py_launcher}")
        py_launcher = None
    if py_launcher:
    ...
    for cmd in candidates:
        ...
        if r.returncode == 0:
            ...
            return cmd
    core.die(TAG, "pyoozをimportできるPython実行系が見つからない。"
             "環境変数D2P_OOZ_PYTHONで明示指定するか、該当のPythonへ"
             "`pip install pyooz`を実行してください"
             "(docs\\REPORT_U18_2026-07-23.md参照)")
...
def _decompress_batch(requests):
    ...
    if proc.returncode != 0:
        core.die(TAG, f"ooz_worker_gpl.pyが異常終了(code={proc.returncode}): "
    ...
    for _ in requests:
        ...
        pos += plen
        if status != 0:
            core.die(TAG, f"ooz解凍エラー: {payload_bytes.decode('utf-8', errors='replace')}")
        results.append(payload_bytes)
    if len(results) != len(requests):
        core.die(TAG, "ooz_worker_gpl.pyの応答数がリクエスト数と一致しない")
    return results
...
def extract_files(pak_path, rel_paths):
    ...
    if missing:
        extra = f" 他{len(missing) - 5}件" if len(missing) > 5 else ""
        core.die(TAG, f"pak内に見つからないパス: {missing[:5]}{extra}")
    ...
    for p in rel_paths:
        ...
        if data is None or len(data) != expected:
            ...
            core.die(TAG, f"解凍後サイズ不一致: {p} got={got} expected={expected}")
        result[p] = data
    return result
