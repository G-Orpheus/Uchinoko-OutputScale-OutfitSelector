...
# 1種類へ)、t00の4096化、単一アトラス、当時はアルファ255固定、ORM Roughness=最大、
...
def inspect_opacity_mask_settings(uexp_bytes):
    """統一元MIのCook済みBasePropertyOverridesからMask設定を読む。

    実データでOpacityMaskClipValue直前2byte目がEBlendMode(1=Masked)、
    anchor自身がclip値であることを確認済み。構造不一致は推測せず返さない。
    """
    hits = [i for i in range(2, len(uexp_bytes) - 4)
            if uexp_bytes[i:i + 4] == _OPACITY_MASK_CLIP_ANCHOR]
    if len(hits) != 1:
        return {"valid": False, "reason": f"clip anchor={len(hits)}件"}
    off = hits[0]
    blend_value = uexp_bytes[off - 2]
    return {
        "valid": blend_value == 1,
        "blend_mode": "Masked" if blend_value == 1 else f"EBlendMode({blend_value})",
        "blend_mode_value": int(blend_value),
        "opacity_mask_clip_value": struct.unpack_from("<f", uexp_bytes, off)[0],
        "opacity_mask": "Base Texture.A",
    }
...
def _patch_mi_base_texture(uasset_path, uexp_path, out_uasset_path, out_uexp_path,
                            ...
                            normal_full_path=None, orm_full_path=None):
    ...
    # Use the structured MI parser for already-unified packages. The older
    # byte-pattern helper can miss valid values after a template rewrite.
    try:
        texture_param_fpis = {
            p["name"]: p["fpi"]
            for p in vp_matparam.list_texture_parameters(data, uexp_bytes_orig)
        }
    except vp_matparam.MatParamError:
        texture_param_fpis = {}

    # 統一済みMIを複製元にする場合、Base TextureのObjectNameは既にt00で
    # `_B`終端ではない。パラメータ値のFPackageIndexを正本として探し、
    # 古いバニラMI向けの`_B`検出は後方互換フォールバックにする。
    base_tex_i = None
    base_fpi = texture_param_fpis.get("Base Texture")
    if base_fpi is None:
        _, base_fpi = _find_named_texture_param_fpi(
            uexp_bytes_orig, names, "Base Texture")
    if base_fpi is not None and base_fpi < 0:
        base_tex_i = -base_fpi - 1
    for i, imp in enumerate(imports):
        if base_tex_i is not None:
            break
        cn = names[imp["class_name_idx"]] if 0 <= imp["class_name_idx"] < len(names) else None
    ...
    # Base Colorに加え、統一MIが参照するNormal/ORMも専用の中立テクスチャへ
    # 再配線する。以前は元Outfitテクスチャの同一パスを平坦化していたため、
    # 防具部分選択時にOutfit配下のテクスチャをpakから除外するとゲーム本体の
    # 元Normal/ORMへフォールバックし、元防具の凹凸・光沢が復活していた。
    normal_full_path = normal_full_path or NEUTRAL_NORMAL_GAME_PATH
    orm_full_path = orm_full_path or NEUTRAL_ORM_GAME_PATH
    import_targets = {
        base_tex_i: (target_full_path, target_short_name, "Base Texture"),
    }
    for param_name in _NORMAL_PARAM_NAMES:
        fpi = texture_param_fpis.get(param_name)
        if fpi is None:
            _, fpi = _find_named_texture_param_fpi(
                uexp_bytes_orig, names, param_name)
        if fpi is not None and fpi < 0:
            import_targets[-fpi - 1] = (
                normal_full_path,
                normal_full_path.rsplit("/", 1)[-1],
                param_name,
            )
    for param_name in _ORM_PARAM_NAMES:
        fpi = texture_param_fpis.get(param_name)
        if fpi is None:
            _, fpi = _find_named_texture_param_fpi(
                uexp_bytes_orig, names, param_name)
        if fpi is not None and fpi < 0:
            import_targets[-fpi - 1] = (
                orm_full_path,
                orm_full_path.rsplit("/", 1)[-1],
                param_name,
            )

    package_targets = {}
    for tex_i, (full_path, _short_name, param_name) in import_targets.items():
        if not (0 <= tex_i < len(imports)):
            raise _OutfitMaterialPatchError(
                f"{param_name}: Texture import indexが範囲外: {tex_i}")
        tex_imp = imports[tex_i]
        outer_fpi = tex_imp["outer_index"]
        if outer_fpi >= 0:
            raise _OutfitMaterialPatchError(
                f"{param_name}: Texture importのouterが非import: {outer_fpi}")
        pkg_i = -outer_fpi - 1
        if not (0 <= pkg_i < len(imports)):
            raise _OutfitMaterialPatchError(
                f"{param_name}: Package import indexが範囲外: {pkg_i}")
        pkg_imp = imports[pkg_i]
        if names[pkg_imp["class_name_idx"]] != "Package":
            raise _OutfitMaterialPatchError(
                f"{param_name}: Texture importのouterがPackageでない")
        previous = package_targets.get(pkg_i)
        if previous is not None and previous != full_path:
            raise _OutfitMaterialPatchError(
                f"同じPackage importへ異なる再配線先が競合: {previous} / {full_path}")
        package_targets[pkg_i] = full_path
    ...
    for full_path, short_name, _param_name in import_targets.values():
        resolve(full_path)
        resolve(short_name)
    name_insert_bytes = b"".join(_encode_name(s_) for s_ in new_names)
    ...
    # 既存import(Package/Texture2D)のObjectNameだけを差し替える。
    ...
    for pkg_i, full_path in package_targets.items():
        patch_i32(imports[pkg_i]["start"] + name_delta + 20,
                  resolved[full_path])
    for tex_i, (_full_path, short_name, _param_name) in import_targets.items():
        patch_i32(imports[tex_i]["start"] + name_delta + 20,
                  resolved[short_name])
...
_ORM_FLAT_VALUE = (0, 204, 255)          # PF_DXT1 (R=Metallic=0, G=Roughness=0.8, B=Occlusion=1)
...
UNIFY_NORMAL_SOURCE_GAME_PATH = (
    "/Game/Pal/Model/Character/Player/Outfit/"
    "SK_Player_Female_Outfit_OldCloth001/v01/"
    "T_Player_Female_Outfit_OldCloth001_v01_M02_N")
UNIFY_ORM_SOURCE_GAME_PATH = (
    "/Game/Pal/Model/Character/Player/Outfit/"
    "SK_Player_Female_Outfit_OldCloth001/v01/"
    "T_Player_Female_Outfit_OldCloth001_v01_M02_M")
NEUTRAL_NORMAL_GAME_PATH = MVP_PACKAGE_PREFIX + "/VP_FlatNormal"
NEUTRAL_ORM_GAME_PATH = MVP_PACKAGE_PREFIX + "/VP_FlatORM"
NEUTRAL_TEXTURE_GAME_PATHS = (
    NEUTRAL_NORMAL_GAME_PATH,
    NEUTRAL_ORM_GAME_PATH,
)
...
def build_unified_mi_variant(pak, ops, tmp_dir):
    ...
    before_params = vp_matparam.list_texture_parameters(
        got[keys[0]], got[keys[1]])
    parent_path = vp_matparam.material_parent_path(got[keys[0]], got[keys[1]])
    opacity_before = inspect_opacity_mask_settings(got[keys[1]])
    if not opacity_before.get("valid"):
        raise _OutfitMaterialPatchError(
            f"統一元MIがMaskedとして確認できない: {opacity_before}")
    print(f"[{TAG}] MI before: Parent={parent_path} "
          f"BlendMode={opacity_before['blend_mode']} "
          f"OpacityMask={opacity_before['opacity_mask']} "
          f"Clip={opacity_before['opacity_mask_clip_value']:.4f}")
    ...
    after_params = vp_matparam.list_texture_parameters(va, ve)
    opacity_after = inspect_opacity_mask_settings(ve)
    if not opacity_after.get("valid"):
        raise _OutfitMaterialPatchError(
            f"統一後MIのMasked設定が壊れた: {opacity_after}")
    print(f"[{TAG}] MI after : Parent={vp_matparam.material_parent_path(va, ve)} "
          f"BlendMode={opacity_after['blend_mode']} "
          f"OpacityMask={opacity_after['opacity_mask']} "
          f"Clip={opacity_after['opacity_mask_clip_value']:.4f}")
    before_by_name = {p["name"]: p["path"] for p in before_params}
    for p in after_params:
        if p["name"] == "Base Texture":
            continue
        print(f"[{TAG}] Texture Parameter: {p['name']} "
              f"{before_by_name.get(p['name'], '(なし)')} -> {p['path']}")
    after_by_name = {p["name"]: p["path"] for p in after_params}
    for param_name in _NORMAL_PARAM_NAMES:
        if param_name in after_by_name \
                and after_by_name[param_name] != NEUTRAL_NORMAL_GAME_PATH:
            raise _OutfitMaterialPatchError(
                f"{param_name}が中立法線を参照していない: {after_by_name[param_name]}")
    for param_name in _ORM_PARAM_NAMES:
        if param_name in after_by_name \
                and after_by_name[param_name] != NEUTRAL_ORM_GAME_PATH:
            raise _OutfitMaterialPatchError(
                f"{param_name}が中立ORMを参照していない: {after_by_name[param_name]}")
    return va, ve, oplog


def _write_neutral_texture_assets(dst_dir):
    """平坦化済みのNormal/ORMをMainShader配下の専用パッケージへ複製する。

    元Outfitと同じパッケージを上書きしないため、部分選択で未選択のバニラ
    防具へ副作用を与えず、かつ選択MIがゲーム本体の元テクスチャへ
    フォールバックすることもない。
    """
    pairs = (
        (UNIFY_NORMAL_SOURCE_GAME_PATH, NEUTRAL_NORMAL_GAME_PATH),
        (UNIFY_ORM_SOURCE_GAME_PATH, NEUTRAL_ORM_GAME_PATH),
    )
    for source_path, target_path in pairs:
        source_rel = source_path[len("/Game/Pal/Model/Character/"):]
        source_file = os.path.join(dst_dir, *source_rel.split("/"))
        with open(source_file + ".uasset", "rb") as f:
            source_uasset = f.read()
        with open(source_file + ".uexp", "rb") as f:
            source_uexp = f.read()
        target_uasset, target_uexp = _clone_mvp_mic_as(
            source_uasset, source_uexp, target_path)
        target_rel = target_path[len("/Game/Pal/Model/Character/"):]
        target_file = os.path.join(dst_dir, *target_rel.split("/"))
        os.makedirs(os.path.dirname(target_file), exist_ok=True)
        with open(target_file + ".uasset", "wb") as f:
            f.write(target_uasset)
        with open(target_file + ".uexp", "wb") as f:
            f.write(target_uexp)
    print(f"[{TAG}] 中立テクスチャを生成: "
          f"VP_FlatNormal=(128,128,255相当), "
          f"VP_FlatORM=(Metallic=0/Roughness=0.8/AO=1)")
...
def _inject_outfit_body_parka_textures(dst_dir, pak, manifest, job=None):
    ...
    # 統一MIの専用中立テクスチャは必ず生成する。元のOutfitテクスチャを
    # 複製元として平坦化した後、MainShader配下へ別パッケージとして置く。
    flatten_targets[UNIFY_NORMAL_SOURCE_GAME_PATH] = _NORMAL_FLAT_VALUE
    flatten_targets[UNIFY_ORM_SOURCE_GAME_PATH] = _ORM_FLAT_VALUE
    _flatten_normal_orm_textures(dst_dir, pak, flatten_targets)
    _write_neutral_texture_assets(dst_dir)
