...
def parse_skin_weight_profiles(data, off):
    ...
    if count != 0:
        raise SkFullParseError(
            f"SkinWeightProfilesData count={count} (>0のエントリ形式は本コーパスで未遭遇・未実装)")
...
def parse_dummy_objs(data, off):
    ...
    if count != 0:
        raise SkFullParseError(f"DummyObjs count={count}(>0は未実装、FPackageIndex配列の意味論未解析)")
    return {'start': off, 'end': off + 4, 'count': count}, off + 4
...
def _mesh_export_end_in_uexp(asset_export, uasset_size):
    ...
    if prefix < 0:
        raise SkFullParseError(
            f"本体exportのSerialOffset({asset_export['serial_offset']})が"
            f"uassetサイズ({uasset_size})未満")
...
def parse_sk_full(uexp_path, uasset_path):
    ...
    if has_cloth:
        raise SkFullParseError("HasClothData()=true (ClothVertexBufferの形式は本コーパスで未遭遇・未実装)")
    ...
    if lod_count != 1:
        raise SkFullParseError(
            f"lod_count={lod_count} (>1のLOD1以降繰り返しパースは本コーパスに実例がなく未検証。"
            "設計はparse_lod_tail/parse_sk_structureの再帰適用で対応可能なはずだが未実装)")
...
def verify(uexp_path, uasset_path, verbose=True):
    ...
    if verbose:
        ...
        print(f"  head_opaque: [0, {full['head_opaque_end']}) = {full['head_opaque_end']} bytes "
              "(Super::Serialize+Bounds/Materials/RefSkeleton、本パーサでは不透明ブロック扱い)")
