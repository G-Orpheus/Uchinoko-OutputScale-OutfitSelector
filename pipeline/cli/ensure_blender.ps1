...
#       # すればよく、理由分岐は不要なため)
#
# Blenderは公式配布元からのみ取得し、固定SHA256で検証する。
...
# 取得物は下の「zip取得」ブロック後に必ずこのハッシュで検証する。
...
try {
    ...
    if ($SourceZip) {
        ...
        Write-Host "  取得元(Blender公式サイト): $DownloadUrl"
        try {
            Invoke-D2PDownloadWithProgress -Url $DownloadUrl -Dest $tempZip
            Write-Host "  取得成功(Blender公式サイト)"
        } catch {
            $downloadErrorMsg = $_.Exception.Message
            Remove-Item $tempZip -Force -ErrorAction SilentlyContinue
            throw "Blender公式サイトからのダウンロードに失敗しました: $downloadErrorMsg"
        }
