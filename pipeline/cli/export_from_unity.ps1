...
try {
    ...
    Write-Host "[警告] [Console]::OutputEncodingの設定に失敗しました(輸出は続行します): $($_.Exception.Message)"
}
...
Write-Host "Unity: $editor (project=$projVer)"
...
if (Test-Path $lock) {
    try {
        ...
        Write-Error ("このプロジェクトはUnityで開かれています。Unityを閉じてから再実行するか、`n" +
            "Unityのメニュー Tools > DiveToPalworld > Export Avatar で手動輸出してください")
        exit 1
    }
}
...
try {
    ...
    if (-not $done) {
        Write-Error "輸出失敗 — ログ参照: $log"
        Select-String -Path $log -Pattern "Exception|error CS" | Select-Object -First 5 | ForEach-Object Line
        exit 1
    }
    Write-Host ""
    Write-Host "輸出完了: $Out"
