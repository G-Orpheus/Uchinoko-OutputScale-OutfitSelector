...
class TestNotFound(_TmpEnvCase):
    ...
    def test_negative_control_message_lists_places_and_manual_howto(self):
        ...
        msg = str(ctx.exception)
        # 探した場所が列挙されている
        self.assertIn(os.path.join(self.hubdir, "editors-v2.json"), msg)
        self.assertIn(os.path.join(self.hubdir, "editors.json"), msg)
        self.assertIn(self.empty_hub_root, msg)
