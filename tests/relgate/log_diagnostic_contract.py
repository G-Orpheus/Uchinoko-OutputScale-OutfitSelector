...
前提(CLAUDE.md「問い合わせからのデバッグ」節、`.devonly\\support\\INQUIRIES.md`、
`.claude\\skills\\bug-report\\SKILL.md`が正本): ユーザーのアバターは絶対に
送ってもらえない。手に入るのは「ログをコピー」の中身だけであり、**ログだけで
原因に到達できないなら、その不具合は永久に直らない**。しかも実例(帽子が
足元に落ちて灰色になる事故)が示すとおり、必要な情報は**成功時のログにも**
残っていなければならない(失敗ログの強化だけでは足りない。INQUIRIES.md記載の
問い合わせの多くは「成功したのに結果が変」という形で来る)。
