...
(step01_import_vrm.py / step02_retarget.py / export_from_unity.ps1)と
UEモード専用(pipeline\ue\ / pipeline\templates\)のみ。判断に迷うものは
含める(漏れの倒れる向き: 含めすぎ=余計にフル実行が走るだけで正しい /
除外しすぎ=誤スキップ。convert.ps1のStep01IgnoredKeysと同じブラックリスト思想)。
...
FP_EXCLUDE_PREFIX = (
    "pipeline/ue/",         # UEモード専用(relgateはnoue経路のみ)
    "pipeline/templates/",  # UEプロジェクトテンプレート(UEモード専用)
)
