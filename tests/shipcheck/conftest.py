...
def pytest_addoption(parser):
    ...
    parser.addoption("--world", default="modtest", choices=["modtest", "panworld"],
                      help="実プレイに使うワールド(既定modtest。panworldはsave_guard必須)")
    ...
    parser.addoption("--allow-convert", action="store_true", default=False,
                      help="安全弁: 指定時のみ実変換(convert.ps1)を許可する(既定は禁止でSKIP)")
    parser.addoption("--allow-machine", action="store_true", default=False,
                      help="安全弁: 指定時のみ実機(Palworld)への接触を許可する(既定は禁止でSKIP)")
    parser.addoption("--run-dir", default=None, help="レポート出力先の上書き(既定: work\\u32_diag\\shipcheck_reports\\<timestamp>)")
...
def _ensure_corpus_job(case_name):
    ...
    job = {
        ...
        "paths": {
            ...
            "ue_project": os.path.join(REPO_ROOT, "ue_project", case_name, "Pal", "Pal.uproject"),
        },
    }
