...
Tier A(このファイルが直接実装。排他資源なし、目標2分以内):
    ...
    A2 文書整合      README.md・docs\\・manual\\ 配下の公開文書に、FBXが対応形式として
                     書かれておらず、「Modular Avatar以外のNDMFプラグイン非対応」の
                     記載があることを守る(CLAUDE.md「対応スコープ」節が根拠)。
    A3 アプリ健全性  app\\build_app.ps1 でGUIがコンパイルでき、起動直後に落ちないことを守る。
...
def gate_a3_app_build_launch(work_root):
    t0 = time.time()
    what = "アプリ(GUI)がコンパイルでき、起動直後(数秒)に落ちないことを守る"
...
def gate_a9_dep_resolver_unit(work_root):
    ...
    tests\\resolver\\test_dep_resolver.py(stdlib unittest、偽台帳フィクスチャは
    ...
    what = ("外部依存resolver(pipeline\\py\\dep_resolver.py, dev#22/#23)のユニット試験が"
            "全て緑であることを守る(tests\\resolver\\test_dep_resolver.py)")
    detail_lines = []
    test_py = os.path.join(REPO_ROOT, "tests", "resolver", "test_dep_resolver.py")
    if not os.path.isfile(test_py):
        return dict(name="A9_dep_resolver_unit", status="FAIL", seconds=time.time() - t0,
                    what=what, detail="テストファイルが存在しない: " + test_py)
    ok = False
    try:
        env = dict(os.environ)
        env["PYTHONDONTWRITEBYTECODE"] = "1"
        ...
        ok = proc.returncode == 0
        detail_lines.append("rc={}".format(proc.returncode))
        detail_lines.append(_tail(proc.stdout + proc.stderr, 3000))
    except Exception:
        detail_lines.append("実行例外:\n" + traceback.format_exc())
    ...
    if args.fast:
        ...
        if scc is None:
            ...
            for case in scc.CASES:
                ...
                try:
                    r = scc.run_case(case, case_work_root, shots_dir)
                except Exception:
