...
やること:
  ...
  4. 展開物**だけ**を使って(`_internal\\pipeline\\cli\\convert.ps1` /
     `_internal\\assets\\tools\\...\\blender.exe` /
     `_internal\\assets\\third_party\\...VRM_Addon...zip`)、変換を1本完走する。
...
DEFAULT_VRM_PATH = os.path.join(REPO_ROOT, "work", "prev_avatars", "shapell", "export", "shapell.fbx")
DEFAULT_HUMANOID_JSON = os.path.join(REPO_ROOT, "work", "prev_avatars", "shapell", "export", "humanoid.json")
...
def run_convert(convert_ps1, job_path, env, log_path, max_retries=20, retry_wait=45):
    """convert.ps1を実行する。マシン全体ミューテックス競合時は
    (WP1/WP2等、他ワークストリームとの並行を想定)45秒間隔でリトライする
    (`work\\relgate\\wp4\\green\\run_green.ps1` / `ship_convert_cases.py` と
    同じ思想)。戻り値: (成功bool, 全stdout+stderrテキスト, 試行回数)"""
    for attempt in range(1, max_retries + 1):
        proc = subprocess.run(
            ...
            ["powershell.exe", "-NoProfile", "-File", convert_ps1, "-Job", job_path, "-EngineMode", "noue"],
...
def main(argv=None):
    ...
    ap.add_argument("--retry-wait", type=int, default=45)
    ...
    try:
        ...
        internal_root = os.path.join(x_stage_root, "_internal")
