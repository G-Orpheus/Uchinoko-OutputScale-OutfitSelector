...
背景: `.devonly\\handover_20260726\\fix_DL_missing_dll.md` の実事故(前提ソフト無しの
...
やること(標準ライブラリのみ、pip追加禁止の既存方針を厳守):
  1. zip内の全 .exe / .pyd エントリのPEインポートテーブルを読む
     (`.devonly\\research\\ue_exit\\pe_imports.py` と同じ手法をここに再実装。
     devtools\\/.devonly\\は非公開のためtests\\shipcheck\\配下に独立実装する)。
...
# .devonly\research\ue_exit\pe_imports.py と同じロジック(読み取り専用の調査
# ツールとして先行実装済み)をここへ再実装する。devtools\/.devonly\は非公開の
