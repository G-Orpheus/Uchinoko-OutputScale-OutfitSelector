...
判定は単語境界必須の正規表現(`\bFBX\b`)を使う。現在の一般配布版に同梱する
マニュアルはルート直下の `manual.md` のみであり、HTMLマニュアルは配布しない。
...
- 現在の一般配布ZIPは、ルート直下に `Uchinoko.exe` を置くフラット構成である。
  マニュアルは `manual.md` のみを同梱する。アプリ内問い合わせ機能と元作者側の
  更新確認機能は対象外であり、出荷検査でも復活していないことを確認する。
- A3の`app\build_app.ps1`は既定で`ico\favicon.ico`をwin32iconに使うが、無くても
  ビルド自体は失敗しない(スクリプト側でTest-Path分岐済み)。
- A4のimport対象は `pipeline\py\*.py` のみ。`pipeline\blender\*.py` と
  `pipeline\ue\*.py` は bpy / Unreal Python 前提で通常のpython環境では絶対に
  importできないため、import確認の対象外(py_compileの構文チェックだけは全件に掛かる)。
...
依存がzip同梱物で閉じているかを検査する。`.devonly\handover_20260726\
fix_DL_missing_dll.md`の実事故(前提ソフト無しのまっさらなWindowsでpython.exe/
ooz.pydがVC++ランタイムを見つけられず即死)の再発防止。標準ライブラリのみ
...
生成されない(旧手順 `build\make_dist.ps1` 直叩きは `.devonly\publish\
publish_steps.md` から削除済み)。「何かを実行し忘れるとチェックが抜ける」経路を
...
3. **pak判定(--pak、WP13 2026-07-27オーナー裁定で最終仕様に統一)**: 判定の正は
   **ただ1つ**、人間がGUI検収で承認した実機記録(`work\u53_cov\
   ...
   `.devonly\publish\release_pak_manifests\` + `devtools\pak_manifest.py`
   エントリ単位比較)はWP13で全廃した。
