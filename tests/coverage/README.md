...
| **UE非依存** | ✔ | `test_ue_independence.py` | ①UE 指紋ファイルが無い(gate D)②ログに UnrealPak/UnrealEditor-Cmd/RunUAT/BuildCookRun が1回も出ない ③**`paths.ue_root`/`ue_project` を丸ごと落とした job.json** でも変換が成立する |
...
| UE モード(`engine_mode: "ue"`) | 本スイートは noue 固定。UE は CLI 開発者向けとして残っているだけ |
...
* `test_input_format` の VRM 7体(**`vrm_alicia051` は DEV_NOTES(28)§1 の出荷ブロッカーを踏むので FAIL する可能性が高い**)
