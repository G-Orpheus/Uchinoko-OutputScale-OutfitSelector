...
SPECIMENS = {
    ...
    "vrm_alicia051": {
        ...
        "why": "【出荷ブロッカーの現物】DEV_NOTES(28)§1。rows=3 で UVセル包含チェックが FATAL",
        "known_issue": "DEV_NOTES 2026-07-25(28) §1: フル変換が FATAL で止まる(未解決)",
    },
    "vrm_seed": {
...
# ---------------------------------------------------------------------------
def make_job(case_name, specimen_key, overrides=None, drop_ue_paths=False,
    ...
    **既存の work\ ディレクトリは一切触らない。**検体ファイル(VRM/FBX)は
    ...
    if not drop_ue_paths:
        # 通常の job.json は GUI が UE のパスも書く(noue では未使用)。
        paths["ue_root"] = r"C:\Program Files\Epic Games\UE_5.1"
        paths["ue_project"] = os.path.join(REPO_ROOT, "ue_project", case_name,
                                           "Pal", "Pal.uproject")
    job["paths"] = paths
...
AXES = {
    ...
    "入力形式:prefab(同名衝突)": {"covered": True, "by": "test_prefab.py::test_prefab_name_collision",
                                   "note": ("別プロジェクトの同名 prefab(Agyo / Jinbe の "
                                            ...
                                            "静的検査なので Unity 不要")},
    "UE非依存": {"covered": True, "by": "test_ue_independence.py",
                 "note": "UE指紋ファイル(gate D)+ ログ内 UE ツール呼び出し + ue_root 無し job での変換"},
